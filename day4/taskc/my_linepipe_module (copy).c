...
Queue *ConstructQueue(int limit) {
    Queue *queue = (Queue*) kmalloc(sizeof(Queue),GFP_KERNEL);
    if (queue == NULL) {
        return NULL;
    }
    if (limit <= 0) {
...
ssize_t linepipe_write(struct file *pfile,const char __user *buffer,size_t length, loff_t *offset){
	...
	temp = (NODE*) kmalloc(sizeof (NODE),GFP_KERNEL);
	temp->data=(DATA*) kmalloc(sizeof (DATA),GFP_KERNEL);
	if(msg){
		kfree(msg);
	}
	msg=(DATA*)kmalloc(sizeof (DATA),GFP_KERNEL);
